...
const TrackOrder = () => {
    ...
    useEffect(() => {
        if (token) {
            fetchOrderDetails();
            const interval = setInterval(fetchOrderDetails, 5000); // Faster polling for real-time feel after OTP
    ...
    return (
        <div className='max-w-[1000px] mx-auto px-4 pt-28 pb-12 animate-fadeIn'>
                ...
                <div className='bg-gradient-to-r from-orange-600 to-orange-400 p-8 text-white'>
                    <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
                        ...
                        <div className='bg-white/20 backdrop-blur-md px-6 py-3 rounded-2xl'>
                            <p className='text-xs font-bold text-orange-50 uppercase tracking-wider mb-1'>Order Total</p>
                            <p className='text-2xl font-black'>₹{Number(order.amount).toFixed(2)}</p>
                        </div>
                    </div>
                </div>
                            ...
                            <div className='bg-gray-50 rounded-3xl border border-gray-100 divide-y divide-gray-100'>
                                {order.items.map((item, index) => (
                                    <div key={index} className='p-4 flex items-center justify-between'>
                                        ...
                                        <p className='font-bold text-gray-700'>₹{Number(item.price * item.quantity).toFixed(2)}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className='space-y-6'>
                            ...
                            <div className='grid gap-6'>
                                ...
                                {order.status !== "Delivered" && order.status !== "Cancelled" ? (
                                    ...
                                    <div className='bg-green-50 p-6 rounded-3xl border-2 border-green-200 border-dashed animate-fadeIn'>
                                        <div className='flex items-center justify-between mb-3'>
                                            <p className='text-xs font-black text-green-600 uppercase tracking-widest'>Order Delivered!</p>
                                            <MdCheckCircle className='text-green-500' size={24} />
                                        </div>
                                        
                                        {/* FlavoHub Delivery Message */}
                                        <div className='bg-white p-5 rounded-2xl border border-green-100 shadow-sm mb-4'>
                                            <p className='text-gray-800 font-bold mb-2 text-sm sm:text-base'>Thank you for ordering from FlavoHub 🙏</p>
                                            <p className='text-gray-700 text-sm mb-1'>Hope you liked the food 😊</p>
                                            <p className='text-gray-700 text-sm mb-3'>Tomorrow also fresh thali available.</p>
                                            {!repliedYes ? (
                                                <button 
                                                    onClick={handleReplyYes}
                                                    className='bg-green-50 px-4 py-2 rounded-xl inline-block border border-green-200 shadow-sm hover:bg-green-100 hover:border-green-300 transition-all active:scale-95 group'
                                                >
                                                    <p className='text-green-700 font-bold text-sm group-hover:text-green-800'>Reply “YES” to get daily menu.</p>
                                                </button>
                                            ) : (
                                                <div className='bg-green-600 px-4 py-2 rounded-xl inline-block shadow-lg shadow-green-100 animate-bounce'>
                                                    <p className='text-white font-bold text-sm flex items-center gap-2'>
                                                        <MdCheckCircle size={16} /> Opted-in Successfully!
                                                    </p>
                                                </div>
                                            )}
                                        </div>

                                        <p className='text-sm text-gray-600 font-medium mb-4'>How was the food and delivery? We'd love to hear from you!</p>
                                        <button
                                            onClick={() => setShowReviewModal(true)}
                                            className='w-full py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-green-100 active:scale-95'
                                        >
            ...
            {showReviewModal && <ReviewModal orderId={orderId} setShowReviewModal={setShowReviewModal} />}
        </div>
    );
};
