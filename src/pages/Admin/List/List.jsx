                            ...
                            <tr key={index} className='bg-gray-50/50 hover:bg-white transition-all group'>
                                <td className='py-4 pl-6 rounded-l-3xl border-y border-l border-transparent hover:border-orange-100'>
                                    <div className='flex items-center gap-4'>
                                        <img className='w-12 h-12 object-cover rounded-xl shadow-sm group-hover:scale-110 transition-transform' src={getImageUrl(item.image)} alt="" />
                                ...
                                <td className='py-4 border-y border-transparent'>
                                    <Link to={`/master-control-gate/add-combo/${item._id}`} className={`px-3 py-1 rounded-full text-xs font-bold transition-all border ${item.addOns && item.addOns.length > 0 ? 'bg-orange-500 text-white border-orange-500 shadow-md shadow-orange-200' : 'bg-gray-50 text-gray-400 border-gray-200 hover:border-orange-200 hover:text-orange-500'}`}>
                                        {item.addOns && item.addOns.length > 0 ? `${item.addOns.length} Combos` : '+ Add'}
                                    </Link>
                                </td>
                                ...
                                <td className='py-4 pr-6 rounded-r-3xl border-y border-r border-transparent text-right'>
                                    <div className='flex items-center justify-end gap-2'>
                                        <Link
                                            to={`/master-control-gate/edit/${item._id}`}
            ...
            <div className="grid grid-cols-1 gap-4 md:hidden">
                {list.map((item, index) => (
                    <div key={index} className='bg-gray-50/50 p-4 rounded-3xl border border-gray-100 flex items-center justify-between gap-4'>
                        <div className='flex items-center gap-4'>
                            <img className='w-14 h-14 object-cover rounded-2xl' src={getImageUrl(item.image)} alt="" />
                            <div>
                                ...
                                <div className='flex gap-2 items-center flex-wrap mt-1'>
                                    ...
                                    <Link to={`/master-control-gate/add-combo/${item._id}`} className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase transition-all border ${item.addOns && item.addOns.length > 0 ? 'bg-orange-500 text-white border-orange-500 shadow-sm' : 'bg-white text-gray-400 border-gray-200 hover:border-orange-200 hover:text-orange-500'}`}>
                                        {item.addOns && item.addOns.length > 0 ? `${item.addOns.length} Combos` : '+ Combo'}
                                    </Link>
                                </div>
